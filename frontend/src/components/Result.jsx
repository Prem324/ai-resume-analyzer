function Result({ result }) {
  ...
  return (
    <div className="animate-in fade-in zoom-in duration-500">
      ...
      <div className="print:hidden bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-5 md:p-8 shadow-2xl mb-8 overflow-hidden">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-white">Full Analysis</h2>
              <p className="text-xs text-slate-500 uppercase tracking-widest font-bold">AI Assessment</p>
            </div>
          </div>

          <button
            onClick={() => window.print()}
            className="w-full md:w-auto group flex items-center justify-center space-x-3 px-6 py-4 bg-white text-slate-900 rounded-xl transition-all hover:bg-slate-100 shadow-xl active:scale-95 cursor-pointer font-black uppercase tracking-widest text-xs"
          >
        ...
        <div className="bg-slate-950/40 rounded-2xl p-4 md:p-8 border border-slate-700/30">
          <div className="text-slate-300 text-sm md:text-lg leading-relaxed whitespace-pre-wrap font-sans">
            {result}
          </div>
        </div>
      </div>
